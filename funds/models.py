...
class Fund(models.Model):
    ...
    fee_rate = models.DecimalField(max_digits=5, decimal_places=2, default=0)  # 申购费率%，0.12=0.12%
    ...
    end_date = models.DateField(null=True, blank=True)           # 停投日
    is_cleared = models.BooleanField(default=False)              # 已清仓
    ...
    def dca_invest_for(self, d: date) -> Decimal:
        """该日的定投扣款金额（毛额，用户实付）。清仓/非定投日/停投后为 0。"""
    ...
    def effective_invested(self, amount) -> Decimal:
        """扣除申购费后实际入账的金额 = amount × (1 − fee_rate/100)，保留 2 位。"""
        rate = Decimal(self.fee_rate or 0)
...
class DailyRecord(models.Model):
    ...
    invested = models.DecimalField(max_digits=12, decimal_places=2, default=0)   # 毛额（用户实付）
